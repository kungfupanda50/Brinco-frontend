<template>
  <div v-if="modelValue" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
    ...
    <div
      class="relative bg-white w-full max-w-5xl max-h-[92vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden"
    >
      <!-- HEADER -->
      <header
        class="px-7 py-5 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0"
      >
        <div>
        ...
        <button
          @click="$emit('update:modelValue', false)"
          class="material-icons text-slate-400 hover:text-slate-700 transition-colors"
        >
          close
        </button>
      </header>
      ...
      >
        ...
        <Transition name="fade">
          <div
            v-if="draggingOver"
            class="absolute inset-0 z-20 bg-cyan-500/10 border-4 border-dashed border-cyan-400 rounded-[2rem] flex items-center justify-center pointer-events-none"
          >
        ...
        <div v-if="cargando" class="flex items-center justify-center py-20">
          <div
            class="w-10 h-10 border-4 border-[#06b6d4] border-t-transparent rounded-full animate-spin"
          ></div>
        </div>
        ...
        <template v-else>
          ...
          >
            <div class="flex gap-3 p-4">
              <!-- MINIATURA fija -->
              <div
                class="shrink-0 w-28 h-28 rounded-xl overflow-hidden bg-slate-200 relative self-start"
              >
                <!-- Video -->
                <video
                  v-if="esVideo(foto.url_archivo)"
                ...
                <button
                  ...
                  title="Eliminar foto"
                  class="absolute top-1 right-1 bg-red-500 hover:bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow"
                >
                ...
                <div class="flex gap-4 min-w-max">
                  ...
                  >
                    ...
                    <div class="flex items-center gap-1 mb-1 pb-1 border-b border-slate-200">
                      <span
                        class="material-icons text-[12px]"
                        :class="colorTexto(cat.color_clase)"
                        >{{ cat.icono }}</span
                      >
                      <span
                        class="text-[9px] font-black uppercase tracking-wider text-slate-400 truncate"
                        >{{ cat.nombre }}</span
                      >
                    </div>
                    ...
                    <label
                      ...
                      :key="tag.id"
                      class="flex items-start gap-1.5 cursor-pointer py-[2px] px-1 -mx-1 rounded hover:bg-white transition-colors"
                    >
                      <input
                        type="checkbox"
                        :checked="fotoTieneTag(foto, tag.id)"
                        @change="toggleTag(foto, tag.id)"
                        class="peer appearance-none w-3 h-3 mt-[2px] border-2 border-slate-300 rounded checked:border-[#06b6d4] checked:bg-[#06b6d4] transition-all shrink-0 cursor-pointer"
                      />
                      <span
                        class="text-[11px] leading-tight text-slate-500 peer-checked:text-[#0891b2] peer-checked:font-semibold transition-colors select-none"
                      >
                        {{ tag.nombre }}
                      </span>
                    </label>
            ...
            <div
              class="px-4 py-2 bg-white border-t border-slate-100 min-h-[32px] flex flex-wrap gap-1.5 items-center"
            >
              <template v-if="foto.tags?.length">
                <span
                  v-for="tag in foto.tags"
                  :key="tag.tag_id"
                  class="inline-flex items-center text-[10px] font-semibold bg-cyan-50 text-cyan-700 border border-cyan-200 rounded-full px-2 py-0.5"
                >
      ...
      <footer class="px-7 py-4 border-t border-slate-100 bg-white shrink-0 flex items-center gap-4">
        <label
          class="flex items-center gap-2 cursor-pointer bg-slate-900 text-white px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-700 transition-colors"
        >
          ...
          <input
            ...
            multiple
            class="hidden"
            @change="subirDesdeInput"
          />
        </label>
        <span v-if="subiendo" class="flex items-center gap-2 text-xs text-slate-400">
          <span
            class="w-4 h-4 border-2 border-[#06b6d4] border-t-transparent rounded-full animate-spin inline-block"
          ></span>
          Subiendo...
        </span>
        <span class="text-xs text-slate-400 ml-auto">
          {{ fotos.length }} foto{{ fotos.length !== 1 ? 's' : '' }} · Tags guardados
          automáticamente
        </span>
      </footer>
    </div>
...
const mapaTagsPlano = computed(() => {
  const m = {}
  categoriasTags.value.forEach((cat) =>
    cat.tags.forEach((t) => {
      m[t.id] = t.nombre
    }),
  )
